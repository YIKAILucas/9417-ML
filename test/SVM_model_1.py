...
# YUCHENG LU takes charge of this SVC model with kernel='poly'
# this code will take about 6 minutes to run it
# read the first 7000 training data and split training data into train_x and train_y
...
# first, to study the value of 'degree':
...
for category in trainClassification:
    ...
    if numOfClass > 1 and numOfOne > 5 and numOfZero > 5:
        paramGrid = {'degree': [1, 2, 3]}  # the grid of parameter 'degree'
        clf = GridSearchCV(estimator=SVC(kernel='poly'), param_grid=paramGrid, cv=5)
...
for category in trainClassification:
    ...
    if numOfClass > 1 and numOfOne > 5 and numOfZero > 5:
        ...
        clf = GridSearchCV(estimator=SVC(kernel='poly', degree=1), param_grid=paramGrid, cv=5)  # set degree=1
        clf.fit(trainX, trainY.values.ravel())
        print(category)
        print(clf.best_score_)
        print(clf.best_params_)
