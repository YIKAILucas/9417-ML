...
# YUCHENG LU takes charge of this MLP model with 'tanh' activation function
# this code will take about 50 minutes to run it
...
for category in trainClassification:
    ...
    if numOfClass > 1 and numOfOne > 5 and numOfZero > 5:
        paramGrid = {'hidden_layer_sizes': [(100,), (100, 30), (100, 60), (100, 90)]}  # the grid of parameter 'degree'
        clf = GridSearchCV(estimator=MLPClassifier(activation='tanh'), param_grid=paramGrid, cv=5)
...
for category in trainClassification:
    ...
    if numOfClass > 1 and numOfOne > 5 and numOfZero > 5:
        ...
        clf = GridSearchCV(estimator=MLPClassifier(activation='tanh'), param_grid=paramGrid, cv=5)  # default (100,)
        clf.fit(trainX, trainY.values.ravel())
        print(category)
        print(clf.best_score_)
        print(clf.best_params_)